...
and `.env <https://pypi.org/project/python-dotenv/>`_ files in the project's root directory, works
with popular testing frameworks like `pytest <https://docs.pytest.org/en/stable/>`_ and `unittest <https://docs.python.org/3/library/unittest.html>`_,
...
(like `.git`, `.hg`, or `.svn` directories) or a custom configuration file (`autopypath.toml`),
and then adding specified subdirectories (such as `src/`, `src/tests/`, `tests/`,  `lib/` or
others defined in configuration files) to :data:`sys.path` at runtime. This allows test scripts

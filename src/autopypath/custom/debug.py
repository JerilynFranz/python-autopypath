...
def configure_pypath(
    *,
    ...
    :param Mapping[str, Literal['dir', 'file']] | None repo_markers: A mapping of file or directory names to
        the marker type ('dir' or 'file') used to identify the repository root.
    ...
    :param Sequence[Path | str] | None windows_paths: A sequence of Windows-specific paths to include in
                        the :var:`sys.path`.
    :param Literal['prepend', 'prepend_highest_priority', 'replace'] | None load_strategy: The
        strategy for loading :var:`sys.path` entries.
